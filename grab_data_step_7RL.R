...
#=======================================================================================
dat <- data.frame()
for (ss in c(1:11)) { # sessions number
...
write.table(dat, '/Users/PC/Dropbox/eloras_rats/data/dat_step6.csv')
...
dat <- read.table('/Users/PC/Dropbox/eloras_rats/data/dat_step6.csv')

all_data <- ddply(dat, .(rat_nr, session, trial_index, group_width, group_dir), function(z){ #int_offset
	#z <- subset(dat, rat_nr==1 & session==1 & trial_index==1)
	...
	cond=0
	if (dim(subset(z, V5=='D_Short_2s' & V3=='Exit'))[1]>0) {cond=1 # find out condition
	...
	poke_choice_lat <- z$time_stamp[max(which(grepl('NPChoiceShift_', z$V5) & z$V3 == "Exit"))]
	

	dur_lat <- z$time_stamp[max(which(grepl('D_', z$V5)& z$V3 == "Exit"))]
	
	response_lat = poke_choice_lat - dur_lat

	return(c(cond, start_light, init_entry, int_offset, first_check, first_check_lat, dur_lat, poke_choice_lat, response_lat))
	}, .progress='text')

colnames(all_data)[c(6:14)] <- c('cond','start_light', 'init_entry', 'int_offset', 'first_check', 'first_check_lat', 'dur_lat', 'poke_choice_lat', 'response_lat')
...
write.table(all_data, 'C:/Users/PC/Dropbox/eloras_rats/data/all_data_step7.csv')
