...
data <- read.csv("/Users/PC/Dropbox/eloras_rats/data/second_half_step7expfiltered.csv")

# Filter the data for the four groups and SE/ME values smaller than or equal to 15
naro_lm <- subset(data, group_width == "NARO" & group_dir == "LM" & SE <= 20 & ME <= 20)
naro_ml <- subset(data, group_width == "NARO" & group_dir == "ML" & SE <= 20 & ME <= 20)
wide_lm <- subset(data, group_width == "WIDE" & group_dir == "LM" & SE <= 20 & ME <= 20)
wide_ml <- subset(data, group_width == "WIDE" & group_dir == "ML" & SE <= 20 & ME <= 20)

# Calculate the mean and standard deviation of SE and ME for each group
mean_SE <- sapply(list(naro_lm, naro_ml, wide_lm, wide_ml), function(group) mean(group$SE))
mean_ME <- sapply(list(naro_lm, naro_ml, wide_lm, wide_ml), function(group) mean(group$ME))
...
ggplot(df_long, aes(x = Groups, y = value, fill = variable)) +
  ...
  labs(title = "Short and Medium Duration Expectancy Latencies by Group for Second Half Lagged Durations", y = "Latencies(s)", fill = "Variable") +
