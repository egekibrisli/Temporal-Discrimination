...
make_trial_index <- function(d) {
  trial_start_stamp <- match(d$V5, 'StarPokeAfterShift') + match(d$V3, 'Entry') - 1
...
#=======================================================================================
dat <- data.frame()
for (ss in c(7:12)) { # sessions number
  dir <- paste('/Users/PC/Dropbox/eloras_rats/data/step7/S', toString(ss),'/', sep='')
  ls <- list.files(dir)
  csv_list <- c()
...
write.table(dat, '/Users/PC/Dropbox/eloras_rats/data/denemestep7/dat_step7exp.csv')
...
dat <- read.table('/Users/PC/Dropbox/eloras_rats/data/denemestep7/dat_step7exp.csv')

all_data <- ddply(dat, .(rat_nr, session, trial_index, group_width, group_dir), function(z){ #int_offset
  #z <- subset(dat, rat_nr==10 & session==1 & trial_index==15)
  tmp1 <- subset(z, V5=='StarPokeAfterShift' & V3=='Entry')
  ...
  cond=0
  if (dim(subset(z, V5=='D_Short_2s' & V3=='Exit'))[1]>0) {cond=1 # find out group_condition
  } else if (dim(subset(z, V5=='D_Medium_4s' & V3=='Exit'))[1]>0) {cond=2
  } else if (dim(subset(z, V5=='D_Long_8s' & V3=='Exit'))[1]>0) {cond=4}
  
  
  int_offset = subset(z, grepl('D_', V5) & V3=='Exit')$time_stamp[1]
  ...
  start_lat <- z$V2[max(which(grepl("D_", z$V5)& z$V3 == "Entry"))]
...
write.table(all_data, 'C:/Users/PC/Dropbox/eloras_rats/data/denemestep7/second_half_step7exp.csv')
...
df <- read.csv("/Users/PC/Dropbox/eloras_rats/data/denemestep7/second_half_step7exp.csv", header = FALSE, col.names = "data")
...
write.csv(df, file = "second_half_step7expcolumned.csv", row.names = FALSE)
...
all_data <- subset(read.csv('C:/Users/PC/Dropbox/eloras_rats/data/denemestep7/second_half_step7expcolumned.csv'))
...
# For subset 2
subset2$SE <- ifelse(subset2$first_poke == 3, 1, 0)
...
write.csv(all_data, 'C:/Users/PC/Dropbox/eloras_rats/data/denemestep7/second_half_step7exp_filtered.csv', row.names = FALSE)
...
# #=======================================================================================
# # Grab all of the  data in one data frame second.
# #=======================================================================================
# dat <- data.frame()
# for (b in c(1:40)) {
...
# 	# }
# 	# EXTRACT second port entry
# 	rt_position <- min(which(rts>0))
# 	# if length(subset(z, grepl('Port',V6))[1,]==0) {second_port_entry<-0}
# 	second_port_entry <- as.character(subset(z, grepl('Port',V6))$V6[rt_position])
...
# 	if (length(second_port_entry)==0) {second_port_entry<-0}
# 	return(c(tp, reward_port, second_port_entry, rts, pellet, pellet_test))
# }, .progress='text')
# colnames(all_data)[c(4:10)] <- c('trial_nr','tp', 'reward_port', 'second_port_entry', 'rt', 'pellet', 'pellet_test')
# all_data$second_port_entry <- ifelse(all_data$second_port_entry=='Port1', 1, ifelse(all_data$second_port_entry=='Port2', 2, 0))
